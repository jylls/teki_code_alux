...
if strcmp(FN,'runTekiv5')
    ...
    mkdir(['data_alux_',day_str])
    mkdir(['data_alux_',day_str,'/random_mats'])
    mkdir(['data_alux_',day_str,'/boop_chords'])
       
    %mkdir(['data_alux_',day_str,'/levels'])
    %levels={};
    %save(['data_alux_',day_str,'/levels/','levels.mat'],'levels');
    %onsets={}
    %boop_chords={}
    %time={}
    %save(['data_alux_',day_str,'/random_tone_matrix_',day_str,'.mat'],'onsets')
    %save(['data_alux_',day_str,'/boop_chords_matrix_',day_str,'.mat'],'boop_chords')
    %save(['time_',day_str,'.mat'],'time')
    disp('Opening Serial, Started..')
end

% while the flag variable is one, the loop continues
while (get(handles.start_button,'UserData')==1)    
    if strcmp(FN,'runTekiv5') %JH stuff
        % if there is data
